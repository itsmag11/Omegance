...
class DDIMSNRControlScheduler(SchedulerMixin, ConfigMixin):
    """
    `DDIMSNRControlScheduler` extends the denoising procedure introduced in denoising diffusion probabilistic models (DDPMs) with
    ...
    def step(
        ...
        def logistic_function(x, L=0.95, U=1.05, x_0=0.0, k=1):
            ...
            if isinstance(new_x, np.ndarray):
                new_x = torch.from_numpy(new_x).to(device=device_, dtype=dtype_)
        ...
        if self.config.prediction_type == "epsilon":
            ...
            pred_epsilon = self.pred_epsilon * omega
            pred_original_sample = (alpha_prod_t**0.5) * sample - ((beta_prod_t**0.5) * self.pred_epsilon - beta_prod_t * sample) / alpha_prod_t ** (0.5)
        else:
            raise ValueError(
                f"prediction_type given as {self.config.prediction_type} must be one of `epsilon`, or"
                " `v_prediction`"
            )
        ...
        if eta > 0:
            ...
            if variance_noise is None:
                ...
                variance_noise_np = np.random.randn(*model_output.shape)
                variance_noise = torch.from_numpy(variance_noise_np).to(device=model_output.device, dtype=model_output.dtype)
            variance = std_dev_t * variance_noise * temperature
