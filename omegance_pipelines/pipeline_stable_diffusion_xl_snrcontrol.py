...
from omegance_pipelines.utils.omega_schedulers import *
...
class StableDiffusionXLSNRControlPipeline(
    DiffusionPipeline,
    ...
    def __call__(
        ...
        omega: float = 0.0,
        omega_start_step: int = 0,
        omega_end_step: int = -1,
        omega_schedule_type: str = None,
        **kwargs,
    ):
        r"""
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                self.noise_pred_over_time[str(i+1)] = noise_pred * omega

                ## -----------------------------
                if omega_schedule_type is None:
                    if t <= omega_start_step and t >= omega_end_step:
                        # print(i)
                        extra_step_kwargs['omega'] = omega
                    else:
                        extra_step_kwargs['omega'] = 0.0
                else:
                    extra_step_kwargs['omega'] = omega_schedule[i]
                    extra_step_kwargs['omega_low'] = 0.9
                    extra_step_kwargs['omega_high'] = 1.1
                ## -----------------------------
                ...
                self.latents_over_time[str(i+1)] = latents
