...
class EulerDiscreteSNRControlScheduler(SchedulerMixin, ConfigMixin):
    """
    Euler scheduler.
    ...
    def step(
        ...
        omega: float = 0.0,
        omega_low: float = 0.95,
        omega_high: float = 1.05,
        ...
        if self.do_omega_rescale:
            omega = logistic_function(omega, L=omega_low, U=omega_high, k=0.1)
        ...
        if self.config.prediction_type == "original_sample" or self.config.prediction_type == "sample":
            raise NotImplementedError(
                f"omega control for prediction_type `original_sample` has not been implemented"
            )
            pred_original_sample = model_output
        elif self.config.prediction_type == "epsilon":
            pred_original_sample = sample - sigma_hat * model_output * omega
        elif self.config.prediction_type == "v_prediction":
            raise NotImplementedError(
                f"omega control for prediction_type `v_prediction` has not been implemented"
            )
    ...
    def disable_omega_rescale(self):
        self.do_omega_rescale = False